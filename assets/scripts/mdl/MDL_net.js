...
*/
     ...
     * @param {string} header
     * @param {C2Function<string, Player>|CFunction<string>} payloadC
    ...
    const addPacketHandler = function thisFun(mode, header, payloadC, addOnce) {
        ...
        if(mode === PacketModes.CLIENT || mode === PacketModes.BOTH) {
            Vars.netClient.addPacketHandler(header, payloadC);
        };
        if(mode === PacketModes.SERVER || mode === PacketModes.BOTH) {
            Vars.netServer.addPacketHandler(header, (player, payload) => payloadC(payload, player));
        };
    }
    .setProp({
        /**
         * @memberof addPacketHandler
         * @type {Array<string>}
         */
        headers: [],
    });
     ...
     * @param {string} url
     * @param {CFunction<Fi>|unset} [callback]
     * @param {CFunction<java.lang.Throwable>|unset} [errCallback]
     * @return {void}
     */
