...
*/
    ...
    /**
     * Variant of {@link addDynaPol} for sync.
     * @param {number} amt
     * @return {void}
     */
    const addDynaPol_global = function(amt) {
        MDL_net.sendPacket(
            PacketModes.BOTH, "lovec-both-pollution-dynamic-add",
            String(amt),
            true,
        );
    }
    .setAnno("init", function() {
        MDL_net.addPacketHandler(PacketModes.BOTH, "lovec-both-pollution-dynamic-add", payload => {
            addDynaPol(Number(payload));
        });
    });
    exports.addDynaPol_global = addDynaPol_global;
    ...
    /**
     * Variant of {@link addLingerPol} for sync.
     * @param {number} amt
     * @return {void}
     */
    const addLingerPol_global = function(amt) {
        MDL_net.sendPacket(
            PacketModes.BOTH, "lovec-both-pollution-lingering-add",
            String(amt),
            true,
        );
    }
    .setAnno("init", function() {
        MDL_net.addPacketHandler(PacketModes.BOTH, "lovec-both-pollution-lingering-add", payload => {
            addLingerPol(Number(payload));
        });
    });
    exports.addLingerPol_global = addLingerPol_global;
