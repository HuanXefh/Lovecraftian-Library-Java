...
*/
  ...
  /* <------------------------------ resource ------------------------------> */
    ...
    /**
     * Whether this content is an intermediate.
     * @param {ContentGn} ct_gn
     * @return {boolean}
     */
    const isIntermediate = function(ct_gn) {
        return matchTag(ct_gn, "ct-intmd", null, true);
    }
    .setCache();
    ...
    /**
     * Whether this content is a waste.
     * @param {ContentGn} ct_gn
     * @return {boolean}
     */
    const isWaste = function(ct_gn) {
        return matchTag(ct_gn, "ct-was", null, true);
    }
    .setCache();
    ...
    const isAuxiliaryFluid = function(liq_gn) {
        return matchTag(liq_gn, "rs-aux", ContentGetModes.RS);
    }
    .setCache();
    ...
    const isNoCapAuxiliaryFluid = function(liq_gn) {
        return matchTag(liq_gn, "rs-aux-nocap", ContentGetModes.RS);
    }
    .setCache();
    ...
    const isDrill = function(blk_gn) {
        return matchCond(blk_gn, "drill", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isHarvester = function(blk_gn) {
        return matchCond(blk_gn, "harvester", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isOreScanner = function(blk_gn) {
        return matchTag(blk_gn, "blk-scan", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isCrop = function(blk_gn) {
        return matchTag(blk_gn, "blk-crop", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isSamePairNoSideBlock = function(blk1_gn, blk2_gn) {
        let blk1 = MDL_content.getCt(blk1_gn, ContentGetModes.BLK);
        let blk2 = MDL_content.getCt(blk2_gn, ContentGetModes.BLK);
    ...
    const isConveyor = function(blk_gn) {
        return (matchCond(blk_gn, "conveyor", ContentGetModes.BLK) && !isCable(blk_gn)) || isStackConveyor(blk_gn);
    }
    .setCache();
    ...
    const isStackConveyor = function(blk_gn) {
        return matchCond(blk_gn, "stackConveyor", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isDuct = function(blk_gn) {
        return matchCond(blk_gn, "duct", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isBridge = function(blk_gn) {
        return matchCond(blk_gn, "bridge", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isGate = function(blk_gn) {
        let blk = MDL_content.getCt(blk_gn, ContentGetModes.BLK);
        return blk != null && (matchCond(blk_gn, "gate", ContentGetModes.BLK) && tryFun(blk.ex_isGateBlk, blk, false));
    }
    .setCache();
    ...
    const isClassicRouter = function(blk_gn) {
        return matchCond(blk_gn, "router", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isMassDriver = function(blk_gn) {
        return matchCond(blk_gn, "massDriver", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isContainer = function(blk_gn) {
        return matchCond(blk_gn, "container", ContentGetModes.BLK) && !isCoreBlock(blk_gn) && !matchTag(blk_gn, "blk-non-cont", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isCoreBlock = function(blk_gn) {
        return matchCond(blk_gn, "core", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isPump = function(blk_gn) {
        return matchCond(blk_gn, "pump", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isPressurePump = function(blk_gn) {
        return matchCond(blk_gn, "pressurePump", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isFluidConduit = function(blk_gn) {
        return matchCond(blk_gn, "conduit", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isFluidContainer = function(blk_gn) {
        return matchCond(blk_gn, "fluidContainer", ContentGetModes.BLK) && !isFluidRouter(blk_gn);
    }
    .setCache();
    ...
    const isFluidRouter = function(blk_gn) {
        return matchCond(blk_gn, "fluidRouter", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isCogwheel = function(blk_gn) {
        return matchTag(blk_gn, "blk-cog", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isGearBox = function(blk_gn) {
        return matchTag(blk_gn, "blk-cog-box", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isTransmissionRod = function(blk_gn) {
        return matchTag(blk_gn, "blk-tor-rod", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isPowerGenerator = function(blk_gn) {
        return matchCond(blk_gn, "generator", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isPowerTransmitter = function(blk_gn) {
        return matchCond(blk_gn, "transmitter", ContentGetModes.BLK) || isCable(blk_gn);
    }
    .setCache();
    ...
    const isCable = function(blk_gn) {
        return matchCond(blk_gn, "cable", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isArmoredCable = function(blk_gn) {
        return matchCond(blk_gn, "armoredCable", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isPowerRelay = function(blk_gn) {
        return matchTag(blk_gn, "blk-pow-relay", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isPowerNode = function(blk_gn) {
        return matchCond(blk_gn, "transmitter", ContentGetModes.BLK) && !isPowerRelay(blk_gn) && !isCable(blk_gn);
    }
    .setCache();
    ...
    const isFactory = function(blk_gn) {
        return (matchCond(blk_gn, "factory", ContentGetModes.BLK) || isMultiCrafter(blk_gn)) && !matchTag(blk_gn, "blk-non-fac", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isMultiCrafter = function(blk_gn) {
        return matchCond(blk_gn, "multiCrafter", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isLightBlock = function(blk_gn) {
        return matchCond(blk_gn, "light", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isProjector = function(blk_gn) {
        return matchCond(blk_gn, "projector", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isRepairer = function(blk_gn) {
        return matchCond(blk_gn, "repairer", ContentGetModes.BLK) && !matchTag(blk_gn, "blk-non-mend", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isShield = function(blk_gn) {
        return matchCond(blk_gn, "shield", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isDefenseWall = function(blk_gn) {
        return matchCond(blk_gn, "wall", ContentGetModes.BLK) && !matchTag(blk_gn, "blk-non-wall", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isTurret = function(blk_gn) {
        return matchCond(blk_gn, "turret", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isTreeBlock = function(blk_gn) {
        return matchTag(blk_gn, "env-tree", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isTallGrassBlock = function(blk_gn) {
        return matchTag(blk_gn, "env-grass-tall", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isDepthOre = function(blk_gn) {
        return matchTag(blk_gn, "env-dpore", ContentGetModes.BLK);
    }
    .setCache();
    ...
    const isDepthLiquid = function(blk_gn) {
        return matchTag(blk_gn, "env-dpliq", ContentGetModes.BLK);
    }
    .setCache();
    ...
    .setAnno("init", function() {
        MDL_event.onLoad(() => {
            this.injuredStas.inSituMap(nameSta => MDL_content.getCt(nameSta, ContentGetModes.STA)).compact();
        });
    });
    ...
    .setAnno("init", function() {
        MDL_event.onLoad(() => {
            this.damagedStas.inSituMap(nameSta => MDL_content.getCt(nameSta, ContentGetModes.STA)).compact();
        });
    });
    ...
    const hasAnyEffect = function(unit, stas_gn) {
        ...
        return stas_gn.some(sta_gn => {
            sta = MDL_content.getCt(sta_gn, ContentGetModes.STA, true);
    ...
    const isNonStatus = function(sta_gn) {
        let sta = MDL_content.getCt(sta_gn, ContentGetModes.STA);
        return sta != null && (
    ...
    const isFadeStatus = function(sta_gn) {
        return matchTag(sta_gn, "sta-fade", ContentGetModes.STA);
    }
    .setCache();
    ...
    const isDeathStatus = function(sta_gn) {
        return matchTag(sta_gn, "sta-death", ContentGetModes.STA);
    }
    .setCache();
