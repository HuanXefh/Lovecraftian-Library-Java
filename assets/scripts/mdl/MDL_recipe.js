...
*/
     ...
     * Used internally in {@link getRcMdl}.
     * @param {string} nameMod
     * @param {string} nameBlk
     * @return {RecipeModule}
     */
    function getRcMdlFromJson(nameMod, nameBlk) {
    ...
    const convertHeader = function(rcMdl, rcHeader) {
        return rcMdl.rc.headerMigration == null ?
            rcHeader :
            tryVal(rcMdl.rc.headerMigration[rcHeader], rcHeader);
    };
     ...
     * @param {RecipeModule} rcMdl
     * @param {ENumber|unset} [mode] - See {@link RecipeKeyResourceModes}.
     * @return {ObjectMap<UnlockableContent, string>}
     */
    ...
    .setProp({
        /**
         * @memberof getKeyCtHeaderMap
         * @param {ObjectMap<UnlockableContent, string>} map
    ...
    const getCategs = function(rcMdl) {
      ...
      while(i < iCap) {
          ...
          if(categ != null && !arr.includes(categ)) {
              arr.push(categ);
          };
          i += 2;
      };
     ...
     */
    const getCategHeadersObj = function(rcMdl) {
        let obj = {};
        let rcHeaders = getRcHeaders(rcMdl);
        getCategs(rcMdl).forEachFast(categ => {
            obj[categ] = [];
            rcHeaders.forEachFast(rcHeader => {
                if(getCateg(rcMdl, rcHeader) == categ) {
                    obj[categ].push(rcHeader);
                };
            }, true);
        }, true);
        return obj;
    };
    exports.getCategHeadersObj = getCategHeadersObj;
     ...
     * @return {string}
     */
    const getCategBundle = function(categ) {
        return MDL_bundle.getTerm("common", "categ-" + categ);
    };
    exports.getCategBundle = getCategBundle;


    /**
    * Gets tooltip text of this recipe, nullable.
    ...
    */
     ...
     * @param {RecipeModule} rcMdl
     * @param {string} rcHeader
     * @return {FFunction<Building, boolean>}
     */
     ...
     * @param {C3Function<UnlockableContent, number, number|null>|unset} [ctC] - `ARGS`: ct, amt, p.
     ...
     * @param {RecipeModule} rcMdl
     * @param {string} rcHeader
     * @return {Array<CFunction<Building>>} - `TUPLE`: updateScr, runScr, craftScr, stopScr, failScr.
     */
