...
*/
    ...
    const createFlash = function(delay, color) {
        return createFade(delay, tryVal(color, Color.white), 0.1, 0.1, 0.0);
    };
     ...
     * @param {boolean|Color|unset} [charaColorArg] - Determines color of the image. The character art will be darkened if this argument is true.
    ...
    const createChara = function(
        ...
        if(!fetchSetting("test-show-error-chara") && !Core.atlas.has(nameMod + "-chara-" + nameChara)) {
            // Do nothing if chara art not found
        } else {
            if(customActs != null) {
     ...
     * @param {number} delay
     * @param {F2Array<string, C0Function>} textScrArr - `ROW`: text, scr.
     * @param {number|unset} [w]
     ...
     * @param {C0Function|unset} [scr] - Called just before the text box is removed.
     * @param {DialogFlowParamObject|unset} [paramObj]
     * @param {F0Function<boolean>|unset} [endF]
     * @return {number}
     */
    const createText = function(delay, dialTup, charaTup, scr, paramObj, endF) {
        ...
        let
            ...
            color = charaTup == null ?
                Color.white :
                MDL_color.getCharaColor(charaTup[0], charaTup[1]),
            dialChara = charaTup == null ?
                "" :
                MDL_bundle.getChara(charaTup[0], charaTup[1]).color(color),
            dialText = dialTup == null ?
                "" :
                MDL_bundle.getDialText(dialTup[0], dialTup[1], dialTup[2]).color(color);

        if(charaTup != null) {
            // `TABLE`: top left character name
        ...
        actor.table(Tex.bar, tb => {
            ...
            tb.table(Styles.none, tb1 => {
                ...
                .size(40.0)
                .tooltip(MDL_bundle.getInfo("lovec", "tt-skip-dial"), true)
                .row();
                tb1.button("L", () => fetchDialog("dialFlowLog").ex_show())
                .size(40.0)
                .tooltip(MDL_bundle.getInfo("lovec", "tt-dial-flow-log"), true)
                .row();
            })
            .right()
            .top();
        })
        .width(getScreenW() * 0.6)
        .height(160.0)
        .row();
    ...
    /**
     * Creates a dialog flow in the scene, which should be defined with {@link newDialogFlow} beforehand.
    ...
    .setProp({
        /**
         * @memberof createFlow
         * @type {ObjectMap<DialogFlowData, number>}
         */
        flowIndMap: new ObjectMap(),
        /**
         * Shows dialog flow.
         * @memberof createFlow
         * @param {DialogFlowData} dialFlowData
         * @return {void}
         */
