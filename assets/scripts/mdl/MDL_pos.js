...
*/
    ...
    const calcSideFrac = function thisFun(b_f, b_t, mode, forceOneSide, useToAsParent) {
        ...
        if(!b_f.block.rotate) {
            ...
            .count(b_t, t => t.build)
            / thisFun.tmpTs.length * (forceOneSide ? 4.0 : 1.0);
        } else {
            switch(mode) {
                ...
                case SideFracModes.SIDE :
                    frac = (
                        (
                            ...
                            LCPos
                            .getTilesRot(b_f.tile, Mathf.mod(b_f.rotation - 1, 4), b_f.block.size, thisFun.tmpTs)
                            .count(b_t, t => t.build)
                        )
                    ) / thisFun.tmpTs.length;
                    break;
                ...
                case SideFracModes.NON_FRONT :
                    frac = LCPos
                    .getTilesEdge(thisFun.tmpTs, b_f.tile, b_f.block.size, false)
                    .count(b_t, t => LCPos.getRotation(b_f.tile, t) === b_f.rotation ? null : t.build)
                     * 4.0 / thisFun.tmpTs.length;
                    break;
                ...
                case SideFracModes.NON_BACK :
                    frac = LCPos
                    .getTilesEdge(thisFun.tmpTs, b_f.tile, b_f.block.size, false)
                    .count(b_t, t => LCPos.getRotation(b_f.tile, t) === Mathf.mod(b_f.rotation + 2, 4) ? null : t.build)
                    * 4.0 / thisFun.tmpTs.length;
                    break;
