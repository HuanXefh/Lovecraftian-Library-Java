...
*/
    ...
    /**
     * Functions called by this method will only get called once in each update.
    ...
    .setProp({
        /**
         * @memberof callOnce
         * @type {ObjectMap<number|string, number>}
         */
     ...
     * @param {CFunction<Unit>|unset} [scr]
     * @return {Unit}
     */
    ...
    const knockback = function(x, y, unit, pow, rad, ang) {
        if(MDL_cond.isUnitInHighAir(unit) || Math.abs(pow) < 0.0001) return;
     ...
     * @param {Bullet} bul
     * @param {number} dmg
     * @return {boolean} Whether this bullet is destroyed.
     */
