...
*/
    ...
    const getTagText = function(strs, ignoreEmpty) {
        ...
        return String.isEmpty(str_fi) && !ignoreEmpty ?
            TmpStateTag.error :
            str_fi;
    };
     ...
     * <br> In the result array, the first layer is for "OR", and the second layer is for "AND".
     * @param {string} str
     * @return {D2Array<string>}
     */
    ...
    /**
     * Gets a list of filters from given keywords, see {@link DB_misc.db.search.tag} for tag definition.
     * @param {Array<string>} keywords
     * @return {Array<FFunction<UnlockableContent, boolean>>}
     */
    const getSearchChecks = function(keywords) {
        let arr = [];
        let arr1 = DB_misc.db["search"]["tag"];
        ...
        keywords.forEachFast(str => {
            ...
            while(i < iCap) {
                ...
                if(str.startsWith(tmpTag)) {
                    ...
                    if(!String.isEmpty(tmpStr)) arr.push(ct => arr1[i + 1](ct, tmpStr));
                    break;
                };
                i += 2;
            };
            if(!isTag) {
                ...
                arr.push(
                    ct => {
                        internalName = ct.name.toLowerCase();
                        rawPlainName = Strings.stripColors(ct.localizedName);
                        plainName = rawPlainName.toLowerCase();
                        return internalName.includes(str)
                            || plainName.includes(str)
                            || (global.lovecUtil.prop.locale === "zh_CN" && LIB_pinyin.fetchPinyin(rawPlainName).toLowerCase().includes(str));
                    },
                );
            };
            isTag = false;
