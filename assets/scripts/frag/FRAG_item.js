...
*/
    ...
    const transItem = function(b, b_t, item, amt, p, isForced) {
        ...
        if(amt < 1) return false;
        if(p == null) p = 1.0;
        let amtTrans = Mathf.maxZero(Math.min(amt.randFreq(p), b.items.get(item), b_t.getMaximumAccepted(item) - b_t.items.get(item)));
        if(amtTrans < 1) return false;

        b_t.handleStack(
            item,
            b.removeStack(item, amtTrans),
            b,
        );
        Call.setItem(b, item, b.items.get(item));
        Call.setItem(b_t, item, b_t.items.get(item));
    ...
    /**
     * Variant of {@link acceptItem2Arr} with probability.
     * @param {Building} b
     * @param {Building} b_f
     * @param {Item3Array} item3Arr
     * @return {boolean}
     */
    const acceptItem3Arr = function(b, b_f, item3Arr) {
        let
            i = 0,
            iCap = item2Arr.iCap(),
            item;
    ...
    exports.acceptItem3Arr = acceptItem3Arr;
     ...
     * @return {boolean}
     */
    const addItem3Arr = function(b, b_f, item3Arr) {
        let
            i = 0,
            iCap = item2Arr.iCap(),
            item,
