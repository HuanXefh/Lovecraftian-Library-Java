...
*/
     ...
     * @return {tmi.recipe.types.RecipeItem}
     */
     ...
     * @param {string} typeStr
     * @param {ContentGn} ct_gn - Owner of the recipe.
    ...
    /**
     * Adds a raw recipe to TMI recipe manager, which should be completed before registration.
    ...
    /**
     * Adds a recipe parser to TMI recipe manager, which is created with {@link extend} from `obj`.
     * @param {Object} obj
    ...
    const addConsPow = function(rawRc, amt) {
        rawRc
        .addMaterialPersec(getTmiCt("power"), amt)
    ...
    const addConsBooster = function(rawRc, ct_gn, amt, boostEffc) {
        rawRc
        .addMaterialPersec(getTmiCt(ct_gn), amt)
     ...
     * @param {tmi.recipe.Recipe} rawRc
     * @param {tmi.recipe.RecipeItemGroup} rcGrp
     * @param {ContentGn} ct_gn
     * @param {number} amt
     * @param {boolean|unset} [isContinuous]
     * @return {tmi.recipe.Recipe}
    ...
    const addProd = function(rawRc, ct_gn, amt, isContinuous) {
        ...
        if(MDL_cond.isWaste(ct_gn)) {
            rcStack.setType(classes.RecipeItemType.GARBAGE);
        };
        return rawRc;
    };
    ...
    const addProdPow = function(rawRc, amt) {
        rawRc
        .addProductionPersec(getTmiCt("power"), amt)
        .setType(classes.RecipeItemType.POWER);
        return rawRc;
    };
    ...
    const addProdHeatErekir = function(rawRc, amt) {
        rawRc
        .addProduction(getTmiCt("heat"), amt)
        .setType(classes.RecipeItemType.POWER)
        .floatFormat();
        return rawRc;
    };
     ...
     * @param {ENumber|unset} [attrRcType]
     ...
     * Should be called on CLIENT LOAD, otherwise consumers added by {@link setConsumer} won't be parsed.
     * @param {Block} blk
     * @param {tmi.recipe.Recipe} rawRc
    ...
    const baseParse = function(blk, rawRc, boostEffc) {
        ...
        // Handle content template fields
        if(checkCreatedByTemp(blk)) {
            // Handle pressure/vacuum production
     ...
     * @param {tmi.recipe.Recipe} rawRc
     * @param {string|CFunction<Table>} str0tableM
        ...
        MDL_event.onLoad(() => {
            terItemMapMap.each((nameItem, terItemMap) => {
                ...
                terItemMap.each((ter, nameRs) => {
                    ...
                    if(rcGrp == null) {
                        rcGrp = new classes.RecipeItemGroup();
                        oreGrpMap.put(rs, rcGrp);
                    };

                    rawRc = makeRawRc("collecting", blk, blk.drillTime / Math.pow(blk.size, 2), true);
    ...
    .setProp({
        /**
         * @memberof regisRc_rangeWallDrill
    ...
    const regisRc_crop = function(blk) {
        ...
        MDL_event.onLoad(() => {
            let
                i = 0,
                iCap = blk.delegee.cropData.iCap(),
                rawRc,
                item,
                amt,
                p;
            ...
            while(i < iCap) {
                ...
                if(item != null && amt * p > 0.0) {
                    rawRc = makeRawRc("factory", blk, blk.ex_calcStageTotalTime(i) - blk.ex_calcStageTotalTime(blk.delegee.cropData[i].stageTo));
    ...
    const regisRc_rainCollector = function(blk) {
        ...
        MDL_event.onLoad(() => {
            ...
            Vars.content.weathers().each(
                ...
                wea => {
                    rawRc = makeRawRc("collecting", blk, 300.0);
        ...
        MDL_event.onLoadDelayTask(5.0, () => {
            CLS_recipe.getBlkRcsMap().get(blk).forEachFast(rc => {
                ...
                if(blk.ex_isSubInsOf("BLK_furnaceRecipeFactory") || blk.ex_isSubInsOf("BLK_electricFurnaceRecipeFactory")) {
                    ...
                    // Stat
                    if(rc.tempReq > 0.0) {
                        addSubInfo(rawRc, MDL_text.getStat(fetchStat("lovec", "blk0heat-tempreq").localized(), Strings.fixed(rc.tempReq, 2), fetchStatUnit("lovec", "heatunits").localized()));
                    };
                    if(isFinite(rc.tempAllowed)) {
                        addSubInfo(rawRc, MDL_text.getStat(MDL_bundle.getTerm("lovec", "temperature-allowed"), Strings.fixed(rc.tempAllowed, 2), fetchStatUnit("lovec", "heatunits").localized()));
                    };
                    if(fuelArr.length > 0) {
                        addSubInfo(rawRc, tb => {
                            tb.row();
    ...
    .setProp({
        /**
         * @memberof regisRc_recipeFactory
         * @type {Seq<UnlockableContent>}
         */
        tmpSeq: new Seq(),
    });
