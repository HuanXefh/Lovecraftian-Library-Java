...
*/
    ...
    /**
     * Completes a row in the dialog flow data.
     * @return {this}
     */
     ...
     * This method always completes the row.
     * @param {number} ind
     * @param {DialogSelectionData} selData
     * @param {number|unset} [w]
     * @param {number|unset} [h]
     * @param {number|unset} [waitTimeS]
     * @return {this}
     */
    CLS_dialogFlowBuilder.prototype.setUselessSelections = function(ind, selData, w, h, waitTimeS) {
        if(waitTimeS == null) waitTimeS = 1.0;

        let texts = [];
        selData.forEachRow(3, (nameMod, nameDial, selInd) => texts.push(MDL_bundle.getDialText(nameMod, nameDial, ind + "-s-" + selInd)), true);
